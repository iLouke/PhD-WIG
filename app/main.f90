program uvlm_demo
   !! ═══════════════════════════════════════════════════════════════════════
   !! UVLM Aerodynamics Demonstration
   !! ═══════════════════════════════════════════════════════════════════════
   !!
   !! This program demonstrates the complete aerodynamic module suite by:
   !!   1. Testing fundamental types (vector3d, quaternion, panel)
   !!   2. Building a simple rectangular flat-plate wing mesh
   !!   3. Running the UVLM solver on that wing
   !!   4. Printing aerodynamic results (lift, drag, circulation)
   !!
   !! Wing geometry (simple test case):
   !!   - Rectangular flat plate (chord x span)
   !!   - Discretized as n_chord x n_span quadrilateral panels
   !!   - Trailing edge at x = chord (downstream edge)
   !!   - Leading edge at x = 0
   !!
   ...
   ! ─── Initialize Logger & Timer ────────────────────────────────────────
   call global_logger%init("uvlm_demo.log", level=LOG_DEBUG)
   call global_logger%msg(LOG_INFO, "=== UVLM Aerodynamics Demo ===")
   call main_timer%start()

   ! ─── Run Tests ────────────────────────────────────────────────────────
   call test_vector3d()
   call test_quaternion()
   call test_biot_savart()
   call test_lapack_solver()
   call test_panel_geometry()
   call run_uvlm_flat_plate()

   ! ─── Finalize ─────────────────────────────────────────────────────────
   call main_timer%stop()
   call global_logger%msg(LOG_INFO, "=== Demo Complete ===")
   call global_logger%msg(LOG_INFO, "Total wall time: "// &
                          trim(real_to_char(main_timer%report(), '(F10.4)'))//" s")
...
contains
   ...
   subroutine run_uvlm_flat_plate()
      ...
      ! Node numbering: row-major (chordwise first, then spanwise)
      !
      !   LE (x=0) ──────────────────── TE (x=chord)
      !   y=span  ┌───┬───┬───┬───┐
      !           │   │   │   │   │
      ...
      ! Each panel is a quad: (i,j)→(i+1,j)→(i+1,j+1)→(i,j+1)
      ! Ordered to match the legacy node numbering convention
   ...
   !> Integer to string helper (local version for main program)
   ...
   subroutine plot_mesh_and_wake(solver)
      ...
      !!   1. Mesh + Wake wireframe (output/uvlm_mesh_wake.png)
      !!   2. Wake colored by circulation (output/uvlm_wake_gamma.png)
